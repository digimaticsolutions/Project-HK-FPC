...
*/ 
final class Ced_StorePickup_Adminhtml_LicenseController extends Mage_Adminhtml_Controller_Action 
{
	...
	public function indexAction() {
		...
		$json = array('success'=>0,'message'=>Mage::helper('storepickup')->__('There is an Error Occurred.'));
		if($postData){
			...
			if ($response && isset($response['hash']) && isset($response['level'])) {
				...
				try {
					...
					{
						$path = Ced_StorePickup_Block_Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_hash';
						$config->saveConfig($path, $response['hash'], 'default', 0);
						$path = Ced_StorePickup_Block_Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_level';
						$config->saveConfig($path, $response['level'], 'default', 0);
						$json['success'] = 1;
						$json['message'] = Mage::helper('storepickup')->__('Module Activated successfully.');
					} else {
						$json['success'] = 0;
						$json['message'] = isset($response['error']['code']) && isset($response['error']['msg']) ? 'Error ('.$response['error']['code'].'): '.$response['error']['msg'] : Mage::helper('storepickup')->__('Invalid License Key.');
					}
				} catch (Exception $e) {
					$json['success'] = 0;
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = (Mage::getStoreConfigFlag(Ced_StorePickup_Block_Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'http://')
    ...
    private function validateAndActivateLicense($urlParams = array())
    {
		...
		$urlParams = array_merge(Mage::helper('storepickup')->getEnvironmentInformation(),$urlParams);
		...
		if (is_array($urlParams) && count($urlParams) > 0) {
			...
			$body = Mage::helper('storepickup')->addParams('',$urlParams);
			$body = trim($body,'?');
		}

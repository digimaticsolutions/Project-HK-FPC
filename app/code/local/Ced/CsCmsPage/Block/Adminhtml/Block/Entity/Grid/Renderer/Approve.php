...
class Ced_CsCmsPage_Block_Adminhtml_Block_Entity_Grid_Renderer_Approve extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract {
	...
	public function render(Varien_Object $row) {
		$html = '';
		if($row->getBlockId()!='' && $row->getIsApprove() == '0') {
			$url =  $this->getUrl('*/*/approved', array('block_id' => $row->getBlockId()));
		...
		if($row->getBlockId()!='' && $row->getIsApprove() == '1') {
			if(strlen($html) > 0) $html .= ' | ';
			$url =  $this->getUrl('*/*/disapproved', array('block_id' => $row->getBlockId()));

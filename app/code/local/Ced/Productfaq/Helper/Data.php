  ...
  public function isAllowedFeedType(SimpleXMLElement $item) {
  	...
  	if(is_array($this->_allowedFeedType) && count($this->_allowedFeedType) >0) {
  		...
  		switch(trim((string)$item->update_type)) {
  			case Ced_Productfaq_Model_Source_Updates_Type::TYPE_NEW_RELEASE :
  			case Ced_Productfaq_Model_Source_Updates_Type::TYPE_INSTALLED_UPDATE :
  				if (in_array(Ced_Canadapost_Model_Source_Updates_Type::TYPE_INSTALLED_UPDATE,$this->_allowedFeedType) && strlen(trim($item->module)) > 0 && isset($cedModules[trim($item->module)]) && version_compare($cedModules[trim($item->module)],trim($item->release_version), '<')===true) {
  					$isAllowed = true;
  					break;
  				}
  			case Ced_Productfaq_Model_Source_Updates_Type::TYPE_UPDATE_RELEASE :
  				if(in_array(Ced_Productfaq_Model_Source_Updates_Type::TYPE_UPDATE_RELEASE,$this->_allowedFeedType) && strlen(trim($item->module)) > 0) {
  					$isAllowed = true;
  					break;
  				}
  				if(in_array(Ced_Productfaq_Model_Source_Updates_Type::TYPE_NEW_RELEASE,$this->_allowedFeedType)) {
  					$isAllowed = true;
  				}
  				break;
  			case Ced_Productfaq_Model_Source_Updates_Type::TYPE_PROMO :
  				if(in_array(Ced_Productfaq_Model_Source_Updates_Type::TYPE_PROMO,$this->_allowedFeedType)) {
  					$isAllowed = true;
  				}
  				break;
  			case Ced_Productfaq_Model_Source_Updates_Type::TYPE_INFO :
  				if(in_array(Ced_Productfaq_Model_Source_Updates_Type::TYPE_INFO,$this->_allowedFeedType)) {
  					$isAllowed = true;
  				}
  				break;
  		}

 ...
 */
final class Ced_Productfaq_Adminhtml_LicenseController extends Mage_Adminhtml_Controller_Action 
{
	...
	public function indexAction() {
		...
		$json = array('success'=>0,'message'=>Mage::helper('productfaq')->__('There is an Error Occurred.'));
		if($postData){
			...
			if ($response && isset($response['hash']) && isset($response['level'])) {
				...
				try {
					...
					{
						$path = Ced_Productfaq_Block_Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_hash';
						$config->saveConfig($path, $response['hash'], 'default', 0);
						$path = Ced_Productfaq_Block_Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_level';
						$config->saveConfig($path, $response['level'], 'default', 0);
						$json['success'] = 1;
						$json['message'] = Mage::helper('productfaq')->__('Module Activated successfully.');
					} else {
						$json['success'] = 0;
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = (Mage::getStoreConfigFlag( Ced_Productfaq_Block_Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'http://')
    ...
    private function validateAndActivateLicense($urlParams = array())
    {
		...
		$urlParams = array_merge(Mage::helper('productfaq')->getEnvironmentInformation(),$urlParams);
		...
		if (is_array($urlParams) && count($urlParams) > 0) {
			...
			$body = Mage::helper('productfaq')->addParams('',$urlParams);
			$body = trim($body,'?');
		}

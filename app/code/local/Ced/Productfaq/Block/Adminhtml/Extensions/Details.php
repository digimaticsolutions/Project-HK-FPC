...
class Ced_Productfaq_Block_Adminhtml_Extensions_Details extends Mage_Adminhtml_Block_Abstract
{
	protected function _toHtml() {
		...
		foreach ($modules as $moduleName=>$releaseVersion)
		{
			...
			$h = Mage::getStoreConfig(Ced_Productfaq_Block_Extensions::HASH_PATH_PREFIX.$m.'_hash');
			for($i=1;$i<=(int)Mage::getStoreConfig(Ced_Productfaq_Block_Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
				{
					$h = base64_decode($h);
				}
				$h = json_decode($h,true); 
			if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && $h['module_name'] == $m && $h['license'] == Mage::getStoreConfig(Ced_Productfaq_Block_Extensions::HASH_PATH_PREFIX.$m)){}
				else{ 
				$args .= $m.'|';
			}	
		}
	...
	protected function _getScriptHtml($params = array(),$height = 140) {
		...
		if(count($params) > 0) $url = Mage::helper('productfaq')->addParams($url,$params,false);

  ...
  */
class Ced_CsStorePickup_Block_Adminhtml_Stores_Edit_Tab_Storehour extends Mage_Adminhtml_Block_Widget_Form
{
	/* public function _construct()
	{
		$this->getLayout()->getBlock('storepickup/adminhtml_store_edit_tab_storehour')
		->setTemplate('storepick/storehour.phtml');
	} */
	
 public function getPickupHour($pickupid)
	{
		$collection = Mage::getModel('storepickup/storepickup')->load($pickupid)->getData();
		//echo $collection['days'];die;
		//$hourInfo = json_decode($collection['days']);
		//print_r($collection['days']);die("hk");
		
		$decode = json_decode($collection['days']);
		$hourInfo = json_decode(json_encode($decode), True);
		//print_r(current($hourInfo));die("hk");
		if(isset($hourInfo))
		{

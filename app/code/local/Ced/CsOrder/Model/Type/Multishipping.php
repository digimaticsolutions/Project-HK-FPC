 ...
 */
class Ced_CsOrder_Model_Type_Multishipping extends Mage_Checkout_Model_Type_Multishipping
{
    /**
    ...
    public function getQuoteShippingAddressesItems()
    {
        ...
        foreach ($addresses as $address) {
            foreach ($address->getAllItems() as $item) {
				...
				$index=$item->getQuoteItemId().'_'.$address->getId().'_'.$item->getProduct()->getId();
				if(isset($items[$index]))
				{
					$items[$address->getId().'_'.$item->getProduct()->getId()]->setQty($items[$index]->getQty()+$item->getQty());
				}
				else
				{
    ...
    public function createOrders()
    {
        ...
        try {
			...
			if($this->getQuote()->getPayment()->getMethod()=='cs_paypal_adaptive'){
				$redirectUrl = $this->getQuote()->getPayment()->getOrderPlaceRedirectUrl();

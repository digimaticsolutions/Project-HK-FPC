...
class Ced_CsTransaction_Block_Adminhtml_Vpayments_Edit_Tab_Addorder extends Ced_CsMarketplace_Block_Adminhtml_Vpayments_Edit_Tab_Addorder
{
	...
	public function getSearchFormHtml() {
		if(Mage::helper('csorder')->isActive())
		{
			...
			$item_commission=Mage::helper('csmarketplace')->getTableKey('item_commission');
			...
			$renderer = new Ced_CsTransaction_Block_Adminhtml_Vorder_Items_Grid_Renderer_Orderid();
			...
			if($type == Ced_CsMarketplace_Model_Vpayment::TRANSACTION_TYPE_DEBIT) {							
				$html.='		            <th class="no-link">'.Mage::helper("csmarketplace")->__("Returning Qty") .'</th>
											 <th class="no-link">'.Mage::helper("csmarketplace")->__("Commission Fee").'</th>
										<th class="no-link">'.Mage::helper("csmarketplace")->__("Vendor Refund").'</th>
										<th class="no-link">'.Mage::helper("csmarketplace")->__("Include Shipping").'</th>
									</tr>
								</thead>
								<tbody>
				';
			...
			{
				$html.='		            <th class="no-link">'.Mage::helper("csmarketplace")->__("Paying Qty") .'</th>
											 <th class="no-link">'.Mage::helper("csmarketplace")->__("Commission Fee").'</th>
			...
			foreach ($collection as $item)
			{
				...
				if($type == Ced_CsMarketplace_Model_Vpayment::TRANSACTION_TYPE_DEBIT) {
					$html .= '<td>'.$item->getQtyReadyToRefund().'</td>';
				}
				else
				{
					$html .= '<td>'.$item->getQtyReadyToPay().'</td>';
				}
				...
				$html .= '<input id="csmarketplace_vendor_orders_'.$item->getId().'" type="hidden" value="'.$this->roundPrice($item->getNetVendorEarn()).'" name="orders['.$item->getOrderId().']['.$item->getId().']"/>';
				...
				{
					$vorder=Mage::getModel('csmarketplace/vorders')->load($item->getParentId());
					$shippingAmount=Mage::helper('cstransaction')->getAvailableShipping($vorder,$type);
					
					if((float)$shippingAmount!=0)
					{
						$arrayShipping[$item->getParentId()]=$item->getParentId();
						$html.='<td><input type="checkbox" checked="checked" name="shippingcheck['.$item->getParentId().']" value="1"><input type="text" readonly="true" class="validate-number-range number-range-0-'.$shippingAmount.'" name="shippings['.$item->getParentId().']" value="'.$shippingAmount.'"></td>';
					}
					else
					{
						$html.='<td>'.$shippingAmount.'</td>';
					}
				}
				$html .= '</tr>';

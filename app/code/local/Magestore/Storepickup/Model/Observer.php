...
class Magestore_Storepickup_Model_Observer
{
    ...
    public function update_shippingaddress($observer)
    {
        ...
        // Zend_debug::dump($storeShipping);
        // Zend_debug::dump($shippingMethod);
        // Zend_debug::dump(isset($storeShipping) && $storeShipping && ($shippingMethod == 'storepickup_storepickup'||$shippingMethod == 'vendor_rates_storepickup_storepickup'));exit;
        if (isset($storeShipping) && $storeShipping && ($shippingMethod == 'storepickup_storepickup'||$shippingMethod == 'vendor_rates_storepickup_storepickup')) {
    ...
    public function adminhtml_sales_order_save_before($observer)
    {
        ...
        try {
            ...
            if ($shippingtime['time'] != null && $shippingtime['date'] != null) {
                $shippingdesct .= '<br/>'.Mage::getModel('storepickup/store')->load($store_id)->getStoreName().'<br/>' . Mage::helper('storepickup')->__('Pickup Time: %s %s ', $date, $shippingtime['time']);
            }
